...
from PyQt5.QtWidgets import (
    ...
    QInputDialog, QMessageBox, QScrollArea, QFrame, QSizePolicy, QProgressBar,
)
...
# ------------------------------------------------------------
# Queue worker — embeds a QUEUE of files one after another, in the
# background, so you can keep using Spikeling. Survives the dialog closing.
# ------------------------------------------------------------
class QueueWorker(QThread):
    # current file path, done chunks, total chunks
    chunk_progress = pyqtSignal(str, int, int)
    # path, added, total, error("" if ok)
    file_done = pyqtSignal(str, int, int, str)
    # nothing left to do
    ...
    def enqueue(self, path, tag=""):
        with self._lock:
            self._queue.append((path, tag))

    def pending(self):
        with self._lock:
            return [p for p, _ in self._queue]
    ...
    def run(self):
        while self._running:
            ...
            if item is None:
                ...
                self.msleep(400)        # idle, wait for new work
                continue
            path, tag = item
            try:
                added, total = self.kb.add_file(
                    path, tag,
                    progress=lambda d, t, p=path: self.chunk_progress.emit(p, d, t))
                self.file_done.emit(path, added, total, "")
            except Exception as e:
                self.file_done.emit(path, 0, 0, str(e))
...
class Spikeling(QMainWindow):
    def __init__(self):
        super().__init__()
        self.memory = Memory()
        self.kb = KnowledgeBase() if HAS_KB else None
        self.active_source = None   # library mode: restrict search to one book, or None=all
        # persistent background queue for embedding books
        self.queue_worker = None
        self.queue_status = {}      # path -> "waiting"/"embedding X/Y"/"done"/"error: ..."
        self._kb_dialog_refresh = None  # set when the Knowledge dialog is open
        if self.kb is not None:
            self.queue_worker = QueueWorker(self.kb)
            self.queue_worker.chunk_progress.connect(self._on_queue_chunk)
            self.queue_worker.file_done.connect(self._on_queue_file_done)
    ...
    def _build_ui(self):
        ...
        kb_btn.clicked.connect(self.open_knowledge)
    ...
    def send(self):
        ...
        # Inject relevant knowledge-base passages (RAG) with source+page citations
        if self.kb is not None:
            try:
                kb_hits = self.kb.search(text, k=4, source=self.active_source)
            except Exception:
                kb_hits = []
            strong = [(src, page, txt) for score, src, page, txt in kb_hits if score > 0.35]
            if strong:
                system += "\n\nREFERENCE PASSAGES (answer from these when relevant, "
                system += "and cite the source/page):\n"
    ...
    # ---- background queue callbacks ----
    def _on_queue_chunk(self, path, done, total):
        name = os.path.basename(path)
        pct = int(done / total * 100) if total else 0
        self.queue_status[name] = f"embedding {done}/{total} ({pct}%)"
        if self._kb_dialog_refresh:
            self._kb_dialog_refresh()

    def _on_queue_file_done(self, path, added, total, err):
        name = os.path.basename(path)
        self.queue_status[name] = f"error: {err}" if err else "done ✓"
        if self._kb_dialog_refresh:
            self._kb_dialog_refresh()
    ...
    def open_knowledge(self):
        if not HAS_KB:
            QMessageBox.warning(self, "Spikeling",
                "Knowledge base unavailable — knowledge.py not found next to the app.")
            return
        dlg = QDialog(self)
        dlg.setWindowTitle("📚 Knowledge Library")
        dlg.setMinimumSize(580, 520)
        lay = QVBoxLayout(dlg)
        lay.addWidget(QLabel("Books & documents Spikeling can search "
        ...
        scope_row.addWidget(QLabel("Answer using:"))
        scope_box = QComboBox()
        scope_row.addWidget(scope_box, 1)
        scope_w = QWidget(); scope_w.setLayout(scope_row)
        lay.addWidget(scope_w)

        lay.addWidget(QLabel("Queue (embeds in background — keep using Spikeling):"))
        queue_list = QListWidget()
        queue_list.setMaximumHeight(120)
        lay.addWidget(queue_list)

        def refresh():
            # loaded books
            cur_scope = scope_box.currentText() if scope_box.count() else "All books"
            listw.clear()
            scope_box.blockSignals(True)
            scope_box.clear()
            scope_box.addItem("All books")
            srcs = self.kb.sources()
            if not srcs:
                listw.addItem("(empty — add a book to begin)")
            for src, cnt in srcs:
                listw.addItem(f"{src}  —  {cnt} chunks")
                scope_box.addItem(src)
            target = self.active_source or "All books"
            i = scope_box.findText(target)
            scope_box.setCurrentIndex(i if i >= 0 else 0)
            scope_box.blockSignals(False)
            # queue status
            queue_list.clear()
            pend = set(os.path.basename(p) for p in
                       (self.queue_worker.pending() if self.queue_worker else []))
            shown = False
            for name, st in self.queue_status.items():
                if st == "done ✓" and name not in pend:
                    queue_list.addItem(f"✓ {name} — done")
                else:
                    queue_list.addItem(f"… {name} — {st}")
                shown = True
            for name in pend:
                if name not in self.queue_status:
                    queue_list.addItem(f"… {name} — waiting")
                    shown = True
            if not shown:
                queue_list.addItem("(queue empty)")
        # let background callbacks refresh THIS dialog while it's open
        self._kb_dialog_refresh = refresh
        refresh()

        def on_scope_changed(idx):
            self.active_source = None if idx <= 0 else scope_box.currentText()
        scope_box.currentIndexChanged.connect(on_scope_changed)

        def add_books():
            from PyQt5.QtWidgets import QFileDialog
            # multi-select: queue several at once
            paths, _ = QFileDialog.getOpenFileNames(
                dlg, "Choose books/documents (you can select several)",
                os.path.expanduser("~"),
                "Documents (*.txt *.md *.pdf);;All files (*)")
            if not paths:
                return
            for p in paths:
                self.queue_status[os.path.basename(p)] = "waiting"
                self.queue_worker.enqueue(p)
            refresh()
        ...
        def delete_selected():
            ...
            if idx < 0 or idx >= len(srcs):
                return
            src = srcs[idx][0]
            if QMessageBox.question(dlg, "Remove book", f"Remove '{src}'?",
                                    QMessageBox.Yes | QMessageBox.No) == QMessageBox.Yes:
                self.kb.delete_source(src)
                self.queue_status.pop(src, None)
                if self.active_source == src:
                    self.active_source = None
                refresh()

        def clear_all():
            if QMessageBox.question(dlg, "Clear", "Remove ALL books?",
                                    QMessageBox.Yes | QMessageBox.No) == QMessageBox.Yes:
                self.kb.clear(); self.active_source = None
                self.queue_status.clear(); refresh()

        row = QHBoxLayout()
        add_btn = QPushButton("Add book(s)…")
        del_btn = QPushButton("Remove selected")
        clear_btn = QPushButton("Clear all")
        close_btn = QPushButton("Close")
        add_btn.clicked.connect(add_books)
        del_btn.clicked.connect(delete_selected)
        clear_btn.clicked.connect(clear_all)
        close_btn.clicked.connect(dlg.accept)
        row.addWidget(add_btn); row.addWidget(del_btn); row.addWidget(clear_btn)
        row.addStretch(); row.addWidget(close_btn)
        lay.addLayout(row)
        dlg.exec_()
        # dialog closed — stop pushing refreshes to it (queue keeps running)
        self._kb_dialog_refresh = None
